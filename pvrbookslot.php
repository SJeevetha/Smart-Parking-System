...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $conn->prepare("INSERT INTO pvrparking (slot_no, name, vehicle_no, in_time, out_time) VALUES (?, ?, ?, ?, ?)");
    $stmt->bind_param("issss", $slot_no, $name, $vehicle_no, $in_time, $out_time);

    if ($stmt->execute()) {
        echo "<script>
                alert('Slot booked successfully!');
                window.location.href='pvrparking.php';
              </script>";
    } else {
        echo "<script>
...
<body>
    ...
    <form method="post" action="pvrbookslot.php">
    ...
    <button class="back-button" onclick="window.location.href='pvrparking.php'">Back</button>
</body>
</html>
