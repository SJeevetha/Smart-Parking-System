...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $conn->prepare("INSERT INTO zudioparking (slot_no, name, vehicle_no, in_time, out_time) VALUES (?, ?, ?, ?, ?)");
    $stmt->bind_param("issss", $slot_no, $name, $vehicle_no, $in_time, $out_time);

    if ($stmt->execute()) {
        echo "<script>
                alert('Slot booked successfully!');
                window.location.href='zudioparking.php';
              </script>";
    } else {
        echo "<script>
...
<body>
    ...
    <form method="post" action="zudiobookslot.php">
    ...
    <button class="back-button" onclick="window.location.href='zudioparking.php'">Back</button>
</body>
</html>
